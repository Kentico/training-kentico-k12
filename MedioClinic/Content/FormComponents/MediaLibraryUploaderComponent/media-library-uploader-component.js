...
(function (mediaLibraryUploaderComponent, undefined) {
    ...
    mediaLibraryUploaderComponent.uploadFile = function (target, url) {
        if (url.length > 0) {
            ...
            var parentForm = mediaLibraryUploaderComponent.getParentForm(target, null);
            var formData = new FormData(parentForm);
            xhr.addEventListener("load", mediaLibraryUploaderComponent.onUploadCompleted, false);
            xhr.addEventListener("progress", mediaLibraryUploaderComponent.onUploadProgressChange, false);
            xhr.addEventListener("error", mediaLibraryUploaderComponent.onUploadFailed, false);
            xhr.open("POST", url);
            xhr.send(formData);
        }
    };

    mediaLibraryUploaderComponent.getParentForm = function (target, bodyElement) {
        ...
        if (target !== bodyElement) {
            ...
            if (parent.tagName === "FORM") {
                ...
                return mediaLibraryUploaderComponent.getParentForm(parent, bodyElement);
            }
        } else {
            return null;
        }
    ...
    mediaLibraryUploaderComponent.onUploadCompleted = function (e) {
    ...
    mediaLibraryUploaderComponent.onUploadProgressChange = function (e) {
        var percentComplete = Math.round(e.loaded * 100 / e.total);
        console.info("Upload progress: " + percentComplete + "%");
    };

    mediaLibraryUploaderComponent.onUploadFailed = function (e) {
